        ...
        for file in os.listdir(loc):
            if file.endswith(".flac") or file.endswith(".wav") or file.endswith(".mp3"):
                ...
                if analyze == True:
                    ...
                    track = Track(str(track_data['title']), str(track_data['artist']), float(track_data['length']), str(analysis_data[1]), float(analysis_data[2]), float(analysis_data[3]), float(analysis_data[4]), float(analysis_data[5]), int(track_data['#bitrate'])/1000, os.path.join(loc,file))
                else:
                    print(track_data)
